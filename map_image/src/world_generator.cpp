...
int Map::create_obstacle_map(int dilation) {
	...
	cv::cvtColor(input_map, gray_map, cv::COLOR_BGR2GRAY);

	// Binarize grayscale map 
	cv::Mat binary_map(gray_map.size(), CV_8UC1);
	cv::threshold(gray_map, binary_map, THRESH_MIN, THRESH_MAX, cv::THRESH_BINARY_INV);
...
std::stack<int> Map::trace_path(std::stack<coord> path) {
	std::stack<int> path_c;
